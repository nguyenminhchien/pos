    ...
    const DepositOrder = (Order) =>
        class DepositOrder extends Order {
            ...
            remove_orderline(line) {
                this.remove_deposit_links(line);
                super.remove_orderline(...arguments);
            }
            remove_deposit_links(line) {
                if (!line.is_container_deposit) return;
                this.orderlines.forEach(function (orderline) {
                    if (orderline.container_deposit_line_id === line.id) {
                        // Orderline.container_deposit_line_id = null;
                    }
                });
            }
